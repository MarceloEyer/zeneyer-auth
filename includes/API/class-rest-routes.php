...
// NÃO importamos com 'use' aqui para evitar conflito se o arquivo não carregar.
// Faremos o require manual dentro da função.
...
class Rest_Routes {
	...
	/**
	 * Handler: Google Login (CORRIGIDO)
	 */
	public static function google_login( WP_REST_Request $request ) {
		$token = $request->get_param( 'id_token' );

		if ( empty( $token ) ) {
			return new WP_Error( 'no_token', 'Google ID Token obrigatório.', array( 'status' => 400 ) );
		}

		// CORREÇÃO DO ERRO 500 AQUI 👇
        // Usamos a constante absoluta para achar o arquivo com certeza.
        ...
        // Verifica de novo se carregou
        if ( ! class_exists( 'ZenEyer\Auth\Auth\Google_Provider' ) ) {
             return new WP_Error( 'class_not_found', 'Erro interno: Classe Google Provider sumiu.', array( 'status' => 500 ) );
        }
    ...
    // --- DEMAIS HANDLERS (Login, Register, Validate, Me) ---
    // (Mantive a lógica de segurança que aprovamos antes)
